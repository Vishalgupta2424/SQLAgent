...
from Myschema import load_schema, store_all_table_structures
from deep_translator import GoogleTranslator

genai.configure(api_key=os.getenv("VISHAL_API_KEY"))
...
7.if query is something like "show students table" use it as "SELECT * FROM students;". 
...
11. always take each input as lowercase strictly, and most important thing is that you should fetch the correct table name, if student table doesn't exists check for students table agr tum table sahi se fetch kar rhe vo hi sabse important task hai jo tumhe karna hai """
